    ...
    @if (Auth::user()->liked($idea))
        <form action="{{ route('ideas.unlike', $idea->id) }}" method="POST">
            ...
            <button type="submit" class="fw-light nav-link fs-6"> <span class="fas fa-heart me-1">
                </span> {{ /*$idea->likes()->count()*/ $idea->likes_count }}</button>
        </form>{{--since we created eager loading for likes inside the index of dashboard controller
            to avoid sql query duplication we can now use the likes_count variable to get all the
            likes count of ideas --}}
    @else
